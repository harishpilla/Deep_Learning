# -*- coding: utf-8 -*-
"""assignment_3_no_attention_CLI.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1506OOwAO22RHgX4KudEaK5zgu3SLMpSg
"""
...
class CustomRNN(object):
    ...
    def getcharactercount(self,char_count):
        corr_perc = np.divide(char_count[:,1], char_count[:,0], out=np.zeros_like(char_count[:,1]), where=char_count[:,0]!=0)*100
        corr_perc = np.round(corr_perc, 2)
        grid = {'Output Character': telugu_characters, 'No.of occurences' : (char_count[:,0]).astype(int), 'Correct Prediction (%)' : corr_perc, 'No. of errors made' : (char_count[:,2]).astype(int)}
        df = pd.DataFrame(grid)
        pd.set_option('display.max_rows', len(telugu_characters))
        print(df)
    
    def test_and_calculate_accuraccy(self, encoder_model, decoder_model, tmp_english_words,
                                     tmp_telugu_words, tmp_encoder_input_data, is_val_accuracy=True , is_visualising=False):
        """
        ...
        preds=[]
        char_count = np.zeros((len(target_token_index), 3))
        for i in range(n):
            ...
            if not is_val_accuracy:
                for char in [z for z in tmp_telugu_words[i].strip()]:
                    char_count[target_token_index[char]][0] += 1
                # Correct characters
                for char in [z for z in tmp_telugu_words[i].strip() if z in output.strip()]:
                    char_count[target_token_index[char]][1] += 1

                # Incorrect characters
                for char in [z for z in tmp_telugu_words[i].strip() if z not in output.strip()]:
                    char_count[target_token_index[char]][2] += 1
        ...
        if is_val_accuracy:
            ...
            self.getcharactercount(char_count)
